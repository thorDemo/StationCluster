...
keywords_path = '%s/templates/CPModel_2/static/keywords.txt' % path      # 关键词目录
...
def list_page(request, list_name):
    ...
    try:
        ...
        raise Http404
    template = loader.get_template('CPModel_2/list.html')
    temp = 0
...
def show_page(request, article_id):
    ...
    try:
        ...
        template = loader.get_template('CPModel_2/show.html')
...
def spider_page(request):
    template = loader.get_template('CPModel_2/logs.html')
    context = {}
    return HttpResponse(template.render(context, request))
